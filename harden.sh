...
DELETED_OPEN_FILES_STATUS="NOT RUN"
...
install_service_dropin() {
    ...
    systemctl is-failed --quiet "$service" && was_failed=1 || true
    if [[ -n "$SSH_SERVICE" && "$service" == "$SSH_SERVICE" ]]; then
    ...
    if [[ "$dropin_was_current" -eq 1 ]]; then
        if [[ "$service" == acct-monthly-report.service && "$was_failed" -eq 1 ]]; then
            if ! verify_acct_monthly_report_runtime; then
                write_systemd_hardening_report "$service" "$activity" "$before" "$before" "$controls" "$classification" "$validation" "failed (one-shot readiness)" "retained for operator review"
                rm -f -- "$dropin_stage"
                return 0
            fi
...
UMask=0027
EOF
    install_service_dropin acct-monthly-report.service 99-hardening "accounting report path, private devices, namespaces and kernel-view isolation" <<'EOF'
[Service]
NoNewPrivileges=yes
PrivateTmp=yes
PrivateDevices=yes
...
apparmor_service_health_check() {
    ...
    [[ "$service" != fail2ban.service ]] || wait_for_fail2ban_runtime "${HARDEN_FAIL2BAN_SERVICE_READINESS_ATTEMPTS:-10}"
}
...
configure_apparmor_vendor_service() {
    ...
    if run_streamed systemctl restart "$service" \
        ...
        printf 'service=%s profile=%s result=kept validation=parser-and-health\n' "$service" "$profile" >> "$APPARMOR_REPORT"
        return 0
    fi
    # The profile was loaded only for this controlled transition. Remove it and
    # restart once to restore the prior unconfined runtime; do not retry loops.
    run_streamed apparmor_parser -R "$profile" || true
    run_streamed systemctl restart "$service" || true
    printf 'service=%s profile=%s result=rolled-back reason=service-health-or-confinement-check-failed\n' "$service" "$profile" >> "$APPARMOR_REPORT"
    log ROLLBACK "AppArmor transition for ${service} failed; unloaded ${profile} and restored the service once"
    return 0
}
...
efivarfs_is_available() {
    local efivars_dir="$1" filesystem_type="${HARDEN_EFIVARS_FSTYPE:-}"
    [[ -d "$efivars_dir" ]] || return 1
    if [[ -z "$filesystem_type" ]]; then
        filesystem_type="$(stat -f -c '%T' "$efivars_dir" 2>/dev/null || true)"
    fi
...
inspect_uefi_mor() {
    local efi_sysfs="${HARDEN_EFI_SYSFS:-/sys/firmware/efi}"
    local efivars_dir="${HARDEN_EFIVARS_DIR:-/sys/firmware/efi/efivars}"
    ...
    elif [[ -n "$lock_value" && "$lock_value" != 0 && "$lock_value" != 1 && "$lock_value" != 2 ]]; then
        UEFI_MOR_STATUS="FAILED-TO-INSPECT (MOR lock has an unknown value)"
        detail="The existing MOR lock value is outside the TCG-defined states; no variable was changed."
    elif [[ "$lock_value" == 1 || "$lock_value" == 2 ]]; then
        UEFI_MOR_STATUS="LOCKED/firmware-controlled (MOR=${mor_value}; lock=${lock_value})"
        detail="Firmware reports MOR control locked; detection only, with no OS-side write."
    elif (( mor_value & 1 )); then
        UEFI_MOR_STATUS="SUPPORTED_ACTIVE (MOR=${mor_value}; lock=${lock_value:-absent})"
        detail="Firmware exposes an active MOR request. It is not made persistent or changed by harden.sh."
    else
        UEFI_MOR_STATUS="SUPPORTED_INACTIVE (MOR=${mor_value}; lock=${lock_value:-absent})"
        detail="Firmware exposes an inactive MOR request. No raw efivarfs write is safe or needed here."
    fi

    if [[ "$MODE" == "apply" ]]; then
        {
            printf 'UEFI Memory Overwrite Request inspection at %s\n\n' "$(timestamp)"
            printf 'status=%s\n' "$UEFI_MOR_STATUS"
        ...
        chmod 0600 "$UEFI_MOR_REPORT"
    else
        log INFO "Would inspect UEFI MOR/MOR-Lock support read-only; no EFI variable would be written"
    fi
    if [[ "$UEFI_MOR_STATUS" == UNSUPPORTED* || "$UEFI_MOR_STATUS" == NOT_APPLICABLE* ]]; then
        record_skip "MOR" "${UEFI_MOR_STATUS}; platform/firmware limitation, not a hardening failure"
    elif [[ "$UEFI_MOR_STATUS" == FAILED-TO-INSPECT* ]]; then
        record_skip "MOR" "${UEFI_MOR_STATUS}; no raw efivarfs write was attempted"
    else
        log INFO "UEFI MOR inspection completed detection-only: ${UEFI_MOR_STATUS}"
    fi
}
...
capture_deleted_open_files() {
    local output="$1"
    lsof -nP +L1 -Fpcuftkn 2>/dev/null | awk '
        /^p/ {pid=substr($0,2); command=user=fd=type=path=links=""; next}
        /^c/ {command=substr($0,2); next}
        /^u/ {user=substr($0,2); next}
        /^f/ {fd=substr($0,2); type=path=links=""; next}
        /^t/ {type=substr($0,2); next}
        /^k/ {links=substr($0,2); next}
        /^n/ {path=substr($0,2); if (pid != "" && path != "") print pid "\t" command "\t" user "\t" fd "\t" type "\t" links "\t" path}
    ' > "$output" || true
}
...
write_deleted_open_report() {
    ...
    temporary="$(mktemp)"
    {
        ...
        printf 'stage=%s\nstatus=%s\n' "$stage" "$DELETED_OPEN_FILES_STATUS"
...
remediate_deleted_open_files() {
    ...
    if [[ "$MODE" == dry-run ]]; then
        log INFO "Would inventory lsof +L1 PID/process/unit ownership and classify only allowlisted service restarts; no file, restart, or reboot state is changed"
        return 0
    fi
    before="$BACKUP_DIR/deleted-open-files-before-remediation.tsv"
    after="$BACKUP_DIR/deleted-open-files-after-remediation.tsv"
    decisions="$BACKUP_DIR/deleted-open-files-decisions.txt"
    : > "$decisions"
    capture_deleted_open_files "$before"
    chmod 0600 "$before" "$decisions"
    if [[ ! -s "$before" ]]; then
        DELETED_OPEN_FILES_STATUS="OK: no deleted open files"
        write_deleted_open_report "$before" "$before" "$decisions" remediation
        log INFO "Deleted-open-file inventory is empty; no service restart is required"
        return 0
    fi
    ...
    while IFS=$'\t' read -r pid process user fd type links file; do
        ...
        if [[ -z "$unit" ]]; then
            printf 'pid=%s unit=unknown classification=unknown decision=no-action reason=no-unambiguous-systemd-service\n' "$pid" >> "$decisions"
        elif [[ "$class" == safe-service ]] && systemctl is-active --quiet "$unit"; then
            restart_units["$unit"]=1
            [[ -n "${seen_units[$unit]+yes}" ]] || printf 'pid=%s unit=%s classification=safe-service decision=restart-once\n' "$pid" "$unit" >> "$decisions"
            seen_units["$unit"]=1
        else
...
Deleted open files : ${DELETED_OPEN_FILES_STATUS}
