...
| Failed-login audit | Die Ubuntu-/Debian-Kette `FAILLOG_ENAB=yes` plus `/var/log/btmp` wird verifiziert; `lastb` bleibt der lesende Nachweis. Bestehendes `pam_faillock` ist die einzige Lockout-Policy. | Keine absichtlichen Fehlversuche gegen Remote-Admin-Konten; nur in einer lokalen, entbehrlichen Testumgebung prüfen. |
